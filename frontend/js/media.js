// media.js
// Handles drag-and-drop + manual file upload for the media library page.
// Displays files (image, video, audio) in #media-grid.

document.addEventListener("DOMContentLoaded", async () => {
    const mediaGrid = document.querySelector("#media-grid");
    const uploadForm = document.querySelector("#upload-form");

    if (!mediaGrid || !uploadForm) {
        // Not on the "Media Library" page
        return;
    }
        ...
        try {
            ...
            media.forEach(item => {
                ...
                if (item.file_type === "image") {
                    ...
                    // For unknown or other file types:
                    mediaItem.innerHTML = `
                      <p>${item.filename}</p>
                      <a href="/api/media/${item.id}" download>Download</a>
                    `;
    ...
    // Initial fetch to populate the grid
    fetchMedia();
});
